...
public struct SynthwaveBackgroundView: View {
    ...
    private func drawGrid(in context: inout GraphicsContext, size: CGSize) {
        guard viewModel.points.count > 1 else { return }

        let offset = viewModel.offset
        let focalLength = viewModel.focalLength

        context.translateBy(x: size.width / 2.0, y: size.height * 0.7)

        for rowIndex in 0..<(viewModel.points.count - 1) {
            let row = viewModel.points[rowIndex]
            let nextRow = viewModel.points[rowIndex + 1]

            for columnIndex in 0..<(row.count - 1) {
                let current = row[columnIndex]
                let right = row[columnIndex + 1]
                let next = nextRow[columnIndex + 1]
                let below = nextRow[columnIndex]

                let p1 = project(current, offset: offset, focalLength: focalLength)
                let p2 = project(right, offset: offset, focalLength: focalLength)
                let p3 = project(next, offset: offset, focalLength: focalLength)
                let p4 = project(below, offset: offset, focalLength: focalLength)
